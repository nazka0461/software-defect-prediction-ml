...
**Module**: 7COM1040 â€” Computer Science Masters Project  
**University**: University of Hertfordshire  
**Project Duration**: 25 February 2026 â€“ 10 April 2026 (6.5 weeks)
...
- **KC1** â€” C++ flight software
- **CM1** â€” NASA spacecraft instrument
- **PC1** â€” NASA flight software
- **JM1** â€” NASA real-time predictive system
- **KC2** â€” C++ scientific software
...
1. **Train/test split** â€” Stratified split (default 80/20) on the binary target `label`, so class proportions are preserved.
...
3. **Transformation pipeline** â€” An imbalanced-learn pipeline is built and fit on the training set:
...
4. **Artifacts** â€” The fitted pipeline, selected feature list, and meta (dropped columns, VIF history) can be saved to a single `.pkl` file (e.g. `src/models/pipeline.pkl`) for reuse in evaluation and API serving.
...
- **Mutual information (MI)** â€” Ranks features by MI with the target; top-*k* (default *k* = 12) are kept.
- **RFE with Random Forest** â€” Recursive Feature Elimination using a Random Forest classifier; the same *k* features are selected.
...
1. **Phase 1 â€” Data Acquisition & Exploratory Analysis**  
...
2. **Phase 2 â€” Feature Engineering & Preprocessing**  
...
3. **Phase 3 â€” Model Development & Evaluation**  
...
5. **Phase 5 â€” Explainability & Thesis Write-Up**  
...
- **`data/`**: Data files (not all are necessarily committed to version control)
  - **`data/raw/`**: Original PROMISE / NASA datasets in ARFF format (e.g. CM1, KC1, PC1, JM1, KC2, MC1, MC2, MW1, PC2â€“PC4, KC3)
...
Optional override if you want to force one deployment bundle path:
...
- Allows runtime selection of dataset/model bundle and adjustable decision threshold from the UI
...
With the environment activated, install all required libraries from `requirements.txt` using uvâ€™s pip interface:
